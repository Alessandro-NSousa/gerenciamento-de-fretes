...
/**
 * REST controller for managing {@link com.companny.gerenciamento.domain.Frete}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<Frete> partialUpdateFrete(@PathVariable(value = "id", required = false) final Long id, @RequestBody Frete frete)
        ...
        Optional<Frete> result = freteRepository
            ...
            .map(existingFrete -> {
                ...
                if (frete.getPrazoDeEntrega() != null) {
                    existingFrete.setPrazoDeEntrega(frete.getPrazoDeEntrega());
                }
