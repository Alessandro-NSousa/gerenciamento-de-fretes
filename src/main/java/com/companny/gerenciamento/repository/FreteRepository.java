...
public interface FreteRepository extends JpaRepository<Frete, Long> {
    ...
    @Query(
        value = "select distinct frete from Frete frete left join fetch frete.cliente left join fetch frete.cidade left join fetch frete.motorista left join fetch frete.caminhao",
        countQuery = "select count(distinct frete) from Frete frete"
    )
    Page<Frete> findAllWithToOneRelationships(Pageable pageable);

    @Query(
        "select distinct frete from Frete frete left join fetch frete.cliente left join fetch frete.cidade left join fetch frete.motorista left join fetch frete.caminhao"
    )
    List<Frete> findAllWithToOneRelationships();

    @Query(
        "select frete from Frete frete left join fetch frete.cliente left join fetch frete.cidade left join fetch frete.motorista left join fetch frete.caminhao where frete.id =:id"
    )
