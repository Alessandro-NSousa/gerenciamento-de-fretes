<template>
  <div>
    <h2 id="page-heading" data-cy="ClienteHeading">
      <span v-text="$t('freightSystemApp.cliente.home.title')" id="cliente-heading">Clientes</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('freightSystemApp.cliente.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'ClienteCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('freightSystemApp.cliente.home.createLabel')"> Create a new Cliente </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && clientes && clientes.length === 0">
      <span v-text="$t('freightSystemApp.cliente.home.notFound')">No clientes found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="clientes">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('freightSystemApp.cliente.nome')">Nome</span></th>
            <th scope="row"><span v-text="$t('freightSystemApp.cliente.cnpj')">Cnpj</span></th>
            <th scope="row"><span v-text="$t('freightSystemApp.cliente.endereco')">Endereco</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="cliente in clientes" :key="cliente.id" data-cy="entityTable">
            ...
            <td>
              <div v-if="cliente.endereco">
                <router-link :to="{ name: 'EnderecoView', params: { enderecoId: cliente.endereco.id } }">{{
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="freightSystemApp.cliente.delete.question" data-cy="clienteDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-cliente-heading" v-text="$t('freightSystemApp.cliente.delete.question', { id: removeId })">
