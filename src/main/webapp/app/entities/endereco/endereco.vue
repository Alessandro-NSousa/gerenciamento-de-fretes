<template>
  <div>
    <h2 id="page-heading" data-cy="EnderecoHeading">
      <span v-text="$t('freightSystemApp.endereco.home.title')" id="endereco-heading">Enderecos</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('freightSystemApp.endereco.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'EnderecoCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('freightSystemApp.endereco.home.createLabel')"> Create a new Endereco </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && enderecos && enderecos.length === 0">
      <span v-text="$t('freightSystemApp.endereco.home.notFound')">No enderecos found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="enderecos">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('freightSystemApp.endereco.cep')">Cep</span></th>
            <th scope="row"><span v-text="$t('freightSystemApp.endereco.logradouro')">Logradouro</span></th>
            <th scope="row"><span v-text="$t('freightSystemApp.endereco.complemento')">Complemento</span></th>
            <th scope="row"><span v-text="$t('freightSystemApp.endereco.numero')">Numero</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="freightSystemApp.endereco.delete.question" data-cy="enderecoDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-endereco-heading" v-text="$t('freightSystemApp.endereco.delete.question', { id: removeId })">
